...
parseInput input =
  ...
  in
    concat $ map (\(y, row) -> map (\(x, _) -> (x,y,0,0)) $ filter (id . snd) row) withIndex


getMaxBounds :: [Cube] -> ((Int, Int), (Int, Int), (Int, Int), (Int, Int))
getMaxBounds cubes =
    (
      (minimum a, maximum a),
      (minimum b, maximum b),
  ...
  where
    a = map (\(a,_,_,_) -> a) cubes
    b = map (\(_,b,_,_) -> b) cubes
    c = map (\(_,_,c,_) -> c) cubes
    d = map (\(_,_,_,d) -> d) cubes
...
runRound input =
  ...
  where
    ((aMin, aMax), (bMin, bMax), (cMin, cMax), (dMin, dMax)) = getMaxBounds input
...
isCloseTo cube1@(a1,b1,c1,d1) cube2@(a2,b2,c2,d2) =
  abs (a1 - a2) <= 1 &&
  abs (b1 - b2) <= 1 &&
  abs (c1 - c2) <= 1 &&
  abs (d1 - d2) <= 1 &&
